#!/usr/bin/env python
from ctlcisco import get_arp,get_cli
import os,sys
import json
import ConfigParser
import argparse
...
def generate_arp_file(host,device_id,fdir):
    conf=CONF.defaults()
    ...
    try:
        ...
        return
    arp=get_arp(cli=cli)
    save_arp(arp,fdir,device_id+".json")
    cli.close()
...
def main():
    ...
    if not os.path.exists(logdir):
        os.makedirs(logdir)
    logging.basicConfig(filename=os.path.join(logdir,'arp.log'), level=logging.WARNING , format="%(asctime)s - %(name)s - %(levelname)s - %(message)s")

    now=datetime.now()
    jsonfile=os.path.join(logdir,'last.topo.json')
    fdir=os.path.join(logdir,'arp',now.strftime('%y%m/%d/%H%M%S'))
    topo=read_topology(jsonfile)
    addrs = get_entry_address(topo)
    for addr in addrs:
        generate_arp_file(addr,addrs[addr],fdir)
